...
function renderCategories(){
  ['expense','income'].forEach(type=>{
    ...
    if(countEl) countEl.textContent = state.categories.filter(c=>c.type===dbType).length;
...
function switchCatTab(type) {
  ...
  document.getElementById('catPanelExpense').style.display = isExpense ? '' : 'none';
  document.getElementById('catPanelIncome').style.display  = isExpense ? 'none' : '';
  document.getElementById('catTabExpense').classList.toggle('active', isExpense);
  document.getElementById('catTabIncome').classList.toggle('active', !isExpense);
}
